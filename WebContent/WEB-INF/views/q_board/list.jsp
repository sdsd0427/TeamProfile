...
<c:set var="qboardList" value="${dataMap.qboardList }" />
    ...
    <section class="content">		
		<div class="card">
			<div class="card-header with-border">
				<button type="button" class="btn btn-primary" id="registBtn" onclick="OpenWindow('registForm.do','글등록',800,700);">질문 등록</button>				
				...
				<table class="table table-bordered text-center" >					
					...
					<c:forEach items="${qboardList }" var="qboard">
						<tr style='font-size:0.85em;'>
							...
							<a href="javascript:OpenWindow('detail.do?from=list&qno=${qboard.qno}','상세보기',800,700);">
							...
							<td>${qboard.writer }</td>
							<td>
								<fmt:formatDate value="${qboard.regDate }" pattern="yyyy-MM-dd"/>
							</td>
							<td ${qboard.status eq '완료' ? 'style="color:red;font-weight:bold"':'' } >${qboard.status }</td>
							<td><span class="badge bg-red">${qboard.viewcnt }</span></td>
						</tr>
					</c:forEach>
				</table>				
			</div>
